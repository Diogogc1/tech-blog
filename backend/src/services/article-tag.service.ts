...
export class ArticleTagService {
  ...
  async create(
    ...
    const articleReference = this.em.getReference(
      Article,
      articleTagPayload.articleId,
    );
    const tagReference = this.em.getReference(Tag, articleTagPayload.tagId);

    const articleTag = ArticleTag.create(
      articleTagPayload,
      articleReference,
      tagReference,
    );
    const response = await this.articleTagRepository.create(articleTag);
    const articleTagResponse: ArticleTagResponse = new ArticleTagResponse(
      response,
    );
  ...
  async findByArticleId(articleId: number) {
    const articleTags =
      await this.articleTagRepository.findByArticleId(articleId);
    const articleTagResponse: ArticleTagResponse[] = articleTags.map(
      (tag) => new ArticleTagResponse(tag),
    );
    return articleTagResponse;
